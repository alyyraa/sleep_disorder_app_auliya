...
def _indonesian_date(value, include_time=False):
    formatted = f"{INDONESIAN_DAYS[value.weekday()]}, {value.day:02d} {INDONESIAN_MONTHS[value.month - 1]} {value.year}"
...
def _page_chrome(canvas, document):
    ...
    canvas.drawString(18 * mm, 14 * mm, f"Page {canvas.getPageNumber()}")
    canvas.restoreState()
...
def build_pdf(report_data, output):
    ...
    document = SimpleDocTemplate(output, pagesize=page_size, leftMargin=18 * mm, rightMargin=18 * mm, topMargin=66 * mm, bottomMargin=42 * mm)
    ...
    story = [Paragraph(report_data["title"], title_style), Paragraph(f"Print Date: {_indonesian_date(generated_at, include_time=True)}<br/>Total Records: {report_data['total_records']}", meta_style), Spacer(1, 5 * mm)]
    if report_data["summary"]:
        summary_rows = [(label, _indonesian_date(generated_at, include_time=True) if label == "Print Date" else value) for label, value in report_data["summary"]]
    ...
    pdf_headers = ["Sleep Disorder Probability (%)" if value == "Class Probability" else value for value in report_data["headers"]]
    ...
    story.append(table)
    signature = Table([[Paragraph(f"Jakarta, {_indonesian_date(generated_at)}<br/><br/>Diketahui oleh,<br/><br/><br/>(Auliya Rahmawati)<br/>Administrator", cell_style)]], colWidths=[62 * mm], hAlign="RIGHT")
    signature.setStyle(TableStyle([("ALIGN", (0, 0), (-1, -1), "RIGHT"), ("VALIGN", (0, 0), (-1, -1), "TOP")]))
    story.extend([Spacer(1, 10 * mm), signature])
